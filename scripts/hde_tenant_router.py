...
ONBOARDING_BOT_USERNAME = os.getenv("HDE_ONBOARDING_BOT_USERNAME", "Humandesigncompanionbot").lstrip("@")
...
# Local guest containers run a ~27B model that can take 60-120s/turn under GPU
# contention; the old hardcoded 35s aborted every real turn ("took too long").
GUEST_TURN_TIMEOUT_SECONDS = float(os.getenv("HDE_GUEST_TURN_TIMEOUT_SECONDS", "180"))
...
# Per-chat welcome rotation: never repeat the same opener twice in a row.
_WELCOME_POOL = [
    "\u2728 *Welcome to your Human Design Sanctuary.*\n\n"
    "You have a guide here to read your chart with you and help you put it to work. "
    "Pick the one that feels right:\n\n"
    "`1) Ember` \u2014 warm, steady, gets to the point\n"
    "`2) Mira` \u2014 bright, curious, loves to explore\n"
    "`3) Something else` \u2014 name it yourself",
    "\u2728 *You\u2019re in. This is your Human Design Sanctuary.*\n\n"
    "A guide stays with you here \u2014 to read your chart and turn it into real decisions. "
    "Which one do you want?\n\n"
    "`1) Ember` \u2014 warm, steady, direct\n"
    "`2) Mira` \u2014 bright, curious, exploratory\n"
    "`3) Your own choice` \u2014 tell me a name",
    "\u2728 *Welcome in. Your sanctuary is set up.*\n\n"
    "Before we read your chart, pick a guide \u2014 the voice that walks beside you:\n\n"
    "`1) Ember` \u2014 steady warmth, straight talk\n"
    "`2) Mira` \u2014 bright and curious, digs in\n"
    "`3) Something else` \u2014 name it",
    "\u2728 *This is your space now.*\n\n"
    "Choose the guide who feels right for reading your chart with you:\n\n"
    "`1) Ember` \u2014 warm, grounded, to the point\n"
    "`2) Mira` \u2014 bright, inquisitive, goes deeper\n"
    "`3) Another name` \u2014 pick your own",
]
_WELCOME_LAST: dict = {}


def guide_choice_prompt(chat_id: int | None = None) -> str:
    """Return a welcome that never repeats the same opener back-to-back."""
    import random
    n = len(_WELCOME_POOL)
    if chat_id is not None:
        idx = (int(_WELCOME_LAST.get(chat_id, 0)) + 1) % n
        if idx == 0:
            idx = random.randrange(n)
        _WELCOME_LAST[chat_id] = idx
    else:
        idx = random.randrange(n)
    return _WELCOME_POOL[idx]


def guide_ready_message(name: str) -> str:
    label = name or "your guide"
    return (
        f"\u2728 *{label} is ready.*\n\n"
        "Two quick things before your first chart:\n"
        "1. What should I call you?\n"
        "2. If you have your birth details on hand \u2014 date, time, and place \u2014 "
        "send them over and I\u2019ll build your chart now. "
        "No worries if you don\u2019t: you can share them any time later."
    )
...
async def process_start_token(client: httpx.AsyncClient, chat_id: int, token: str) -> None:
    ...
    async with async_session_factory() as db_session:
        ...
        await send_telegram_message(
            client,
            chat_id,
            "🧪 Beta testing note: Michael (a Human) and Ned (an AI) may review this test conversation, generated chart artifacts, stuck states, and feedback to improve the Sanctuary bot experience. This applies to this staging test round and is separate from production customer privacy."
        )
        await send_telegram_message(client, chat_id, guide_choice_prompt(chat_id))
...
async def handle_user_chat(client: httpx.AsyncClient, chat_id: int, text: str) -> None:
    ...
    if bot_instance.status == "awaiting_guide_choice":
        ...
        if not guide_name:
            await send_telegram_message(
                client,
                chat_id,
                "Still waiting on the guide name \u2014 `1) Ember` or `2) Mira` works, "
                "or name it yourself. If this message is about something else, "
                "just say that and we\u2019ll start there.",
            )
            return
    ...
    try:
        resp = await client.post(resolved_url, json={"text": text}, timeout=GUEST_TURN_TIMEOUT_SECONDS)
        if resp.status_code == 200:
            data = resp.json()
        ...
        try:
            resp = await client.post(new_resolved_url, json={"text": text}, timeout=GUEST_TURN_TIMEOUT_SECONDS)
            if resp.status_code == 200:
                data = resp.json()
                if BUDGET_GUARD is not None and budget_decision is not None:
                    await BUDGET_GUARD.reconcile_chat_turn(budget_decision, extract_usage_tokens(data))
