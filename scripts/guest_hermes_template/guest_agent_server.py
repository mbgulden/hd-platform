...
# Naming guard (HFG-NAMING-GUARD): dev/test names must never be filed as chart
# profiles. Data-driven (not per-guest code) + extensible via env.
# Exact-match, case-insensitive, applied to the CLEANED name.
BLOCKED_PERSON_NAMES = {
    n.strip().lower()
    for n in (
        ["michael gulden", "michael", "becca", "becca gulden",
         "test", "tester", "test guest", "guest test", "qa", "dev"]
        + os.getenv("GUEST_BLOCKLIST_NAMES", "").replace(",", " ").split()
    )
    if n.strip()
}


def is_blocked_person_name(name: str) -> bool:
    """True if `name` is a dev/test placeholder that must not be filed as a
    chart profile. Compares the cleaned form so 'Michael.' / 'michael GULDEN'
    are caught. Real client names that merely resemble these (e.g. a client
    actually named Mr. Test) are a known false-positive tradeoff of an
    exact-match blocklist — extend via GUEST_BLOCKLIST_NAMES, never by code."""
    cleaned = clean_person_name(name).lower().strip(" .")
    if not cleaned:
        return False
    return cleaned in BLOCKED_PERSON_NAMES
...
def handle_pdf_report_request(text: str) -> dict | None:
    ...
    for turn in reversed(load_history()):
        ...
        if not details:
            continue
        _n1 = (details.get("name") or "").strip()
        if not _n1:
            _p1 = load_person_profile(default_person_slug()) if default_person_slug() else {}
            _n1 = _p1.get("name") if _p1 and not is_generic_profile(default_person_slug(), _p1) else ""
        name = (_n1 or "Sanctuary Guest").strip()
...
# ── Third-party name/relationship detection ──────────────────────────
# A birth-data message that names someone other than the user must be filed
# under that person, not merged into the user's own (default) profile.
_REL_FAMILY = ("mother", "mom", "mum", "dad", "papa", "father", "sister", "brother",
               "wife", "husband", "spouse", "son", "daughter", "aunt", "uncle",
               "niece", "nephew", "cousin", "grandmother", "grandfather", "grandma",
               "grandpa", "grandson", "granddaughter", "stepmother", "stepfather",
               "boyfriend", "girlfriend", "partner")
_REL_FRIEND = ("friend", "friends", "neighbor", "neighbour", "colleague",
               "coworker", "classmate")
_ALL_REL = sorted(set(_REL_FAMILY) | set(_REL_FRIEND), key=len, reverse=True)
_REL_RE = re.compile(r"\b(" + "|".join(re.escape(w) for w in _ALL_REL) + r")\b", re.I)
_BAD_NAME_STARTS = {"born", "was", "is", "san", "new", "denver", "seattle", "austin",
                    "portland", "chicago", "boston", "miami", "omaha", "tulsa", "los",
                    "june", "may", "february", "march", "april", "july", "august",
                    "september", "october", "november", "december", "january"}
_BAD_WORDS = {"she", "he", "i", "you", "we", "they", "this", "that", "someone", "the",
              "person", "my", "a", "an", "it", "who", "what", "called", "name"} | set(_ALL_REL)


def _collect_name(s: str) -> str:
    """Collect a leading proper-name token from s (skips spaces/commas first)."""
    if not s:
        return ""
    i = 0
    ...
    while i < len(s):
        ...
        if c.isalpha() or c in " '\u2019-":
            ...
            if (i + 1 < len(s) and s[i + 1].isupper() and s[i + 1].isalpha()
                    and (i + 2 >= len(s) or not s[i + 2].isalpha())):
                i += 2
            else:
                break
        else:
            break
    raw = s[start:i].strip()
    raw = re.split(r"\s+(?:born|was|is|at|in|on|chart|profile|design|reading|date|time|place|for|the|,)\b",
                   raw, maxsplit=1, flags=re.I)[0]
    raw = re.sub(r"['\u2019]s$", "", raw)
    return raw.strip(" .,;'-\u2019")


def _valid_name_token(s: str) -> bool:
    s = (s or "").strip()
    if len(s) < 2 or not s[0].isupper():
        return False
    return s.split()[0].lower() not in _BAD_NAME_STARTS and s.split()[0].lower() not in _BAD_WORDS


def _relation_of(low: str):
    if any(re.search(rf"\b{re.escape(w)}\b", low) for w in _REL_FAMILY):
        return "family"
    if any(re.search(rf"\b{re.escape(w)}\b", low) for w in _REL_FRIEND):
        return "friends"
    return None


def detect_third_party_name(text: str):
    """Return (name, relationship) for a non-self person, else (None, None)."""
    raw = (text or "").strip()
    if not raw:
        return None, None
    low = raw.lower()
    if re.match(r"^\s*(?:my name is|i am|i'm|this is me|my chart|for me|under my profile|here are my|my birth|my details|my info)", low):
        return None, None
    rel = _relation_of(low)
    for m in _REL_RE.finditer(raw):
        name = _collect_name(raw[m.end():])
        if _valid_name_token(name):
            return name, (rel or "other")
    for mm in re.finditer(r"\b([A-Z][A-Za-z]+(?:['\u2019]s)?(?:\s[A-Z][A-Za-z]+)*)\s+(?:was|is)\s+born\b", raw):
        name = _collect_name(" " + mm.group(1))
        if _valid_name_token(name):
            return name, (rel or "other")
    for pat in (r"\b(?:add|create|build|file|save|new)\s+(?:a\s+)?(?:profile|chart|design|reading)\s*,?\s*(?:for\s+)?",
                r"\b(?:chart|profile|design)\s+for\s+"):
        mm = re.search(pat, raw, re.I)
        if mm:
            name = _collect_name(raw[mm.end():])
            if _valid_name_token(name):
                return name, (rel or "other")
    return None, None
...
def generate_one_shot_chart_from_details(text: str) -> dict | None:
    ...
    if not details:
        return None
    third_name, third_rel = detect_third_party_name(text)
    if third_name:
        # A different person was named — file under them, never the user's profile.
        name = third_name
        relationship_type = third_rel if third_rel in ("family", "friends") else "other"
    else:
    ...
    save_people_index(index)
    birth = {"birth_date": details["birth_date"], "birth_time": details["birth_time"], "location": details["location"]}
    generated = generate_chart_for_birth_details(name, slug, birth, relationship_type=relationship_type)
    note = f" Filing {name}\u2019s chart under {relationship_type}." if relationship_type != "personal" else ""
    return {"response": "I generated the chart from the birth details you sent." + note + "\n" + generated.get("response", "")}
...
def handle_natural_profile_update(text: str) -> dict | None:
    ...
    if explicit_name:
        name = explicit_name
        relationship_type = third_rel if third_rel in ("family", "friends") and third_name else "personal"
    elif third_name:
        name = third_name
        relationship_type = third_rel if third_rel in ("family", "friends") else "other"
    else:
        name = (existing_name if existing_name and existing_name.lower() not in {"my human design", "user", "sanctuary guest", "guest"} else "") or "Sanctuary Guest"
        relationship_type = "personal"
    slug = slugify_person_name(name)
    ...
    birth = {"birth_date": details["birth_date"], "birth_time": details["birth_time"], "location": details["location"]}
    return generate_chart_for_birth_details(name, slug, birth, relationship_type=relationship_type)
...
async def handle_progressive_chart_intake(text: str) -> dict | None:
    ...
    if not pending and CHART_INTENT_RE.search(text or ""):
        ...
        if explicit_name and is_blocked_person_name(explicit_name):
            logger.warning("NAMING-GUARD: blocked dev/test name at chart intent: %r", explicit_name)
            return {"response": "That name looks like a dev/test placeholder, so I won’t file it as a chart profile. Use a real first and last name."}
    ...
    if stage == "person_name":
        ...
        if is_blocked_person_name(person_name):
            logger.warning("NAMING-GUARD: blocked dev/test name at name capture: %r", person_name)
            return {"response": "That name looks like a dev/test placeholder, so I won’t file it as a chart profile. Use a real first and last name."}
...
def _log_build_identity() -> None:
    """Drift canary (HFG-DRIFT-CANARY): log this server's own build identity at boot.

    Emits md5 + line count of the running guest_agent_server.py. fleet_audit
    compares this against the .build marker written by fleet_sync; a mismatch
    means a guest is running a non-canonical build. Best-effort: any failure
    logs a warning and never blocks startup.
    """
    try:
        import hashlib
        import os
        path = os.path.abspath(os.path.expanduser("~/.hermes/guest_agent_server.py"))
        ...
        marker = None
        try:
            with open(os.path.join(os.path.dirname(path), ".build"), "r") as mf:
                marker = mf.read().strip()
        except OSError:
            pass
        logger.info(
            "BUILD-IDENTITY path=%s md5=%s lines=%d marker=%s",
            path, md5.hexdigest(), lines, marker,
        )
    except Exception as exc:  # noqa: BLE001 - canary must never break boot
        logger.warning("BUILD-IDENTITY could not be computed: %s", exc)
...
def build_usage(input_text: str, output_text: str) -> dict:
    ...
    return {
        "provider": os.getenv("GUEST_MODEL_PROVIDER", "qwen27b-fred-local"),
        "model": os.getenv("GUEST_MODEL_NAME", "local-qwen-27b-q8-fred"),
